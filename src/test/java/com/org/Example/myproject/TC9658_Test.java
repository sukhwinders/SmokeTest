...
public class TC9658_Test {
	...
	public void searchingfor_conectTOproduct() throws Exception {
		...
		driver.findElement(By.xpath("//input[@id='txtIdValue0']")).sendKeys(
				UPCproduct);
		Thread.sleep(8000);
		driver.findElement(By.id("btnProductSearch")).click();
		Thread.sleep(5000);
		Assert.assertTrue(driver.findElement(By.xpath("//td[5]/button"))
				.isDisplayed(), "product is not avilable");
		driver.findElement(
				By.xpath("//button[contains(.,'Connect to Product')]")).click();
		driver.findElement(
				By.xpath("//input[@id='txt_UPProductRelationship_Name']"))
				.sendKeys(Product);
		if (System.getProperty("os.name").toLowerCase().contains("win")) {
